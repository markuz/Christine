...
class christineEvents(Singleton):
	...
	to be used mainly by plugins that insert their functions to any event they want/need.

	We want to replace the current scheme based on generating "events" by changes on 
	christineConf, that is not optimal, and it doesn't support a variety of interesting events,
	namely "onPlay", "onStop", etc.

	This should probably be implemented with DBus library, but could make Christine less portable
	...
	def addToEvent(self, event, func, *args):
		'''
		Adds a function to a given event (event) to be triggered by its ocurrence as
		func(user_data...)
